...
results = {}
if not new_snp.empty:
    new_stocks = list(new_snp["symbol"])
    ...
    if stock_diff != -1:
        ...
        new_with_moves = data_util.calc_moves(new_stocks, df)
        data = json.dumps(new_with_moves.to_dict(orient="records"))
        print(f"New stocks' data:")
        print(data)
        print()
        ...
        data = json.dumps(remaining_update)
        print(f"Stocks to be updated: ")
        print(data)
        print()
        post_update = requests.post(urls.prod_update_stocks_url, data=data, headers=headers)
        results["post_update"] = post_update.status_code

        # post new snp_list
        data = json.dumps(new_snp.to_dict(orient="records"))
        print(f"New list to be sent via post request: ")
        print(data)
        print()
        post_list = requests.post(urls.prod_replace_snplist, data=data, headers=headers)
        results["post_list"] = post_list
        ...
        data = json.dumps(daily)
        print(f"Stocks to be updated (no new stocks): ")
        print(new_stocks)
        print()
        post_update = requests.post(urls.prod_update_stocks_url, data=data, headers=headers)
        results["post_update"] = post_update.status_code
    ...
    # save new snp list to csv
    new_snp.to_csv("data/csv/snp_list.csv")
    print("new snp_list.csv saved")
    print()
    ...
    print("Error: scrape.get_list() failed...")
